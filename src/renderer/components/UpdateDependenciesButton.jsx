...
const UpdateDependenciesButton = () => {
  ...
  const title = language === "ruby" ? "Update synvert ruby dependencies and sync synvert ruby snippets" : "Update synvert javascript dependencies and sync synvert javascript snippets";
  ...
  return (
    <button type="button" className="btn btn-primary btm-sm" onClick={updateDependencies} disabled={updateDependenciesButtonDisabled} title={title}>{text}</button>
  );
}
